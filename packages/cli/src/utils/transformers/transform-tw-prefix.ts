...
export const transformTwPrefixes: Transformer = async ({ sourceFile, config }) => {
    ...
    .forEach((node) => {
      ...
      if (node.getArguments()[0]?.isKind(SyntaxKind.StringLiteral)) {
        ...
        if (defaultClassNames) {
          defaultClassNames.replaceWithText(`"${applyPrefix(defaultClassNames.getText()?.replace(/"/g, ""), config.tailwind.prefix)}"`);
        }
      }
        ...
        node
          ...
          .forEach((node) => {
            node.getDescendantsOfKind(SyntaxKind.PropertyAssignment).forEach((node) => {
              ...
              if (classNames) {
                classNames?.replaceWithText(`"${applyPrefix(classNames.getText()?.replace(/"/g, ""), config.tailwind.prefix)}"`);
              }
            });
          });
      }
  ...
  sourceFile.getDescendantsOfKind(SyntaxKind.JsxAttribute).forEach((node) => {
    if (node.getNameNode() === "className") {
      // className="..."
      if (node.getInitializer()?.isKind(SyntaxKind.StringLiteral)) {
        const value = node.getInitializer();
        if (value) {
          value.replaceWithText(`"${applyPrefix(value.getText()?.replace(/"/g, ""), config.tailwind.prefix)}"`);
        }
      }
      ...
      if (node.getInitializer()?.isKind(SyntaxKind.JsxExpression)) {
        ...
        if (callExpression) {
          ...
          callExpression.getArguments().forEach((node) => {
            if (node.isKind(SyntaxKind.ConditionalExpression) || node.isKind(SyntaxKind.BinaryExpression)) {
              node.getChildrenOfKind(SyntaxKind.StringLiteral).forEach((node) => {
                node.replaceWithText(`"${applyPrefix(node.getText()?.replace(/"/g, ""), config.tailwind.prefix)}"`);
              });
            }
            ...
            if (callExpression) {
              ...
              callExpression.getArguments().forEach((arg) => {
                ...
                if (arg.isKind(SyntaxKind.StringLiteral)) {
                  arg.replaceWithText(`"${applyPrefix(arg.getText()?.replace(/"/g, ""), config.tailwind.prefix)}"`);
                }
              });
            }
          }
